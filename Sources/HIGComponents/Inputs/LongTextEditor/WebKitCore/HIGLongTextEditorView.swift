...
///
/// `HIGLongTextEditorView` supports the display of HTML content using
...
/// of an email.
///
/// The appearance of the editor can be customized thanks to the ``HIGLongTextEditorView/injectAdditionalCSS(_:)-406n9`` method.
/// The editor provides information about the style of the currently selected text or of
/// the text at the insertion point in the ``HIGLongTextEditorView/selectedTextAttributes``
/// property.
/// Many functions are available to update the style, such as ``HIGLongTextEditorView/bold()``.
public class HIGLongTextEditorView: PlatformView {
    ...
    public weak var delegate: HIGLongTextEditorViewDelegate?

    /// The style of the text currently selected in the editor view.
    public private(set) var selectedTextAttributes = HIGLongTextUITextAttributes()
    ...
    public private(set) var webView: HIGLongTextHTMLWebView!
...
public extension HIGLongTextEditorView {
...
public extension HIGLongTextEditorView {
    private func setUpWebView() {
        webView = HIGLongTextHTMLWebView(frame: .zero, configuration: WKWebViewConfiguration())
...
extension HIGLongTextEditorView: WKNavigationDelegate {
    public func webView(
        _ webView: WKWebView,
        ...
        case .linkActivated:
            if let url = navigationAction.request.url, delegate?.higLongTextEditorView(self, shouldHandleLink: url) == true {
...
#if canImport(UIKit)
extension HIGLongTextEditorView: UIScrollViewDelegate {
...
extension HIGLongTextEditorView: ScriptMessageHandlerDelegate {
    func editorDidLoad() {
        javaScriptManager.isDOMContentLoaded = true
        delegate?.higLongTextEditorViewDidLoad(self)
    }

    func contentDidChange(_ text: String) {
        rawHTMLContent = text
        delegate?.higLongTextEditorViewDidChange(self)
    }
    ...
    func selectedTextAttributesDidChange(_ selectedTextAttributes: HIGLongTextUITextAttributes?) {
        guard let selectedTextAttributes else {
            return
        }

        self.selectedTextAttributes = selectedTextAttributes
        delegate?.higLongTextEditorView(self, selectedTextAttributesDidChange: selectedTextAttributes)
    }

    func caretRectDidChange(_ position: CGRect) {
        delegate?.higLongTextEditorView(self, caretPositionDidChange: position)
    }

    func caretPositionDidChange(_ caretRect: CGRect) {
        delegate?.higLongTextEditorView(self, caretPositionDidChange: caretRect)
    ...
    func selectionDidChange(_ selection: String) {
        ...
        delegate?.higLongTextEditorView(self, selectionDidChange: selection)
    }
}

// MARK: - JavaScriptManagerDelegate

extension HIGLongTextEditorView: JavaScriptManagerDelegate {
    func javascriptFunctionDidFail(error: any Error, function: String) {
        delegate?.higLongTextEditorView(self, javascriptFunctionDidFail: error, whileExecuting: function)
    }
}
