...
- `HIGTabBar`
- `HIGSidebar`
- `HIGAdminShell` / `HIGAdminShellStyle` — admin app shell with brand chrome; `.sidebar` (Remark base), `.iconRail` (iconbar), `.topBar` (topbar); tokens via `theme.adminShell`
- `HIGToolbar`

#### Admin catalog expansion (planned families)

Full Remark Admin Template parity is tracked in `Docs/ADMIN_TEMPLATE_PORT.md`. Priority follow-ons: additional admin shell styles (`.topIcon`, `.centered`, `.drawer`) and widgets.
