...
- `HIGPricingCardTokens` defines padding, spacing, radius, borders, and title/price/feature fonts for ``HIGPricingCard``.
...
- `HIGAdminShellTokens` defines brand/content padding, brand font, sidebar widths, icon-rail metrics, top-bar metrics, and centered max width for ``HIGAdminShell``.
...
- `HIGLightboxTokens` defines chrome padding, media height, control size, indicators, and fonts for ``HIGLightbox``.
