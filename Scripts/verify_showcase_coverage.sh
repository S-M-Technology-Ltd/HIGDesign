...
showcase_key_for() {
    case "$1" in
        ...
        HIGCard) echo card ;;
        HIGPanel) echo panel ;;
        HIGPanelGroup) echo panelGroup ;;
        HIGCounter) echo counter ;;
        HIGWidget) echo widget ;;
        HIGRating) echo rating ;;
        HIGTestimonial) echo testimonial ;;
        HIGRibbon) echo ribbon ;;
        HIGPricingCard) echo pricingCard ;;
        HIGChatBubble) echo chatBubble ;;
        HIGComment) echo comment ;;
        HIGCover) echo cover ;;
        HIGCoverSolidBackgroundView) echo cover ;;
        HIGImageOverlay) echo imageOverlay ;;
        HIGImageOverlayCaptionPanelView) echo imageOverlay ;;
        HIGColorSelector) echo colorSelector ;;
        HIGColorOption) echo colorSelector ;;
        HIGSocialButton) echo socialButton ;;
        HIGImageFrame) echo imageFrame ;;
        HIGImageFramePlaceholderView) echo imageFrame ;;
        HIGTreeView) echo treeView ;;
        HIGTreeNode) echo treeView ;;
        HIGReorderableList) echo reorderableList ;;
        HIGReorderableListItem) echo reorderableList ;;
        HIGCalendar) echo calendar ;;
        HIGDashboardGrid) echo dashboardGrid ;;
        HIGVideoPlayer) echo videoPlayer ;;
        HIGCoachMark) echo coachMark ;;
